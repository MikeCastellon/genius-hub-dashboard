...
export default function Services() {
  ...
  const inputClass = 'w-full px-3 py-2 rounded-xl border border-zinc-200 bg-white text-sm text-zinc-900 placeholder:text-zinc-300 focus:outline-none focus:border-blue-300 focus:ring-2 focus:ring-blue-500/10'
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        {isAdmin && (
          <button onClick={() => setShowAddForm(!showAddForm)}
            className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-gradient-to-r from-blue-500 to-sky-400 text-white text-xs font-semibold shadow-md shadow-blue-500/25 hover:shadow-lg transition-all">
            <Plus size={14} />
        ...
        <div className="glass rounded-2xl p-4 mb-6 space-y-3">
          ...
          <div className="flex gap-2 pt-1">
            <button onClick={handleAdd} disabled={adding}
              className="px-4 py-2 rounded-xl bg-gradient-to-r from-blue-500 to-sky-400 text-white text-xs font-semibold shadow-sm hover:shadow-md transition-all disabled:opacity-50">
        ...
        <table className="w-full min-w-[600px]">
          ...
          <tbody>
            {displayed.map(service => (
              <tr key={service.id} className="border-b border-zinc-50 hover:bg-blue-50/20 transition-colors">
                <td className="px-4 py-3 text-xs font-medium text-zinc-700">{service.name}</td>
                <td className="px-4 py-3">
                  {editingId === service.id && isAdmin ? (
                    <select value={editCategory} onChange={e => setEditCategory(e.target.value)}
                      className="w-full px-2 py-1 text-xs rounded-lg border border-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-500/10">
                      {SERVICE_CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
                    </select>
                  ) : (
                    <span className="text-[11px] px-2.5 py-1 rounded-full bg-blue-50 text-blue-600 border border-blue-100 font-medium">{service.category}</span>
                  )}
                </td>
                <td className="px-4 py-3 text-right">
                  {isAdmin && editingId === service.id ? (
                    <input type="number" value={editPrice} onChange={e => setEditPrice(e.target.value)}
                      className="w-20 px-2 py-1 text-xs rounded-lg border border-blue-300 focus:outline-none text-right" step="0.01" />
                  ) : (
                ...
                <td className="px-4 py-3 text-right">
                  {isAdmin && editingId === service.id ? (
                    <input type="number" value={editDuration} onChange={e => setEditDuration(e.target.value)}
                      className="w-16 px-2 py-1 text-xs rounded-lg border border-blue-300 focus:outline-none text-right" placeholder="—" />
                  ) : (
                    ...
                    {editingId === service.id ? (
                      ...
                      <div className="flex gap-1.5 justify-end">
                        <button onClick={() => startEdit(service)}
                          className="text-[11px] font-medium text-blue-500 hover:text-blue-600 transition-colors">Edit</button>
          ...
          return (
            <div key={service.id} className="glass rounded-2xl overflow-hidden">
              <div className="flex items-center gap-3 p-3 cursor-pointer"
                ...
                <span className="text-sm font-bold text-blue-600 shrink-0">{formatCurrency(service.price)}</span>
              ...
              {isExpanded && isAdmin && (
                <div className="border-t border-zinc-100 p-3 bg-zinc-50/30">
                  {isEditing ? (
                    <div className="space-y-2">
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <input type="number" value={editPrice} onChange={e => setEditPrice(e.target.value)}
                            className="w-full px-2 py-1.5 text-xs rounded-lg border border-blue-300 focus:outline-none" step="0.01" />
                        </div>
                        <div>
                          <label className="text-[10px] font-semibold text-zinc-400 uppercase">Duration (min)</label>
                          <input type="number" value={editDuration} onChange={e => setEditDuration(e.target.value)}
                            className="w-full px-2 py-1.5 text-xs rounded-lg border border-blue-300 focus:outline-none" placeholder="—" />
                        </div>
                      </div>
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => startEdit(service)}
                        className="flex-1 py-2 rounded-xl bg-blue-50 text-blue-600 text-xs font-semibold hover:bg-blue-100 transition-colors">Edit</button>
                      <button onClick={() => toggleActive(service.id, service.active)}
